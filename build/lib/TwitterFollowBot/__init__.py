...
class TwitterBot:
    ...
    def wait_on_action(self):
        ...
        if wait_time > 0:
            print("เลือกเวลาระหว่าง %d และ %d - รอ %d วินาที จะเริ่มทำงาน" % (min_time, max_time, wait_time))
    ...
    def bot_setup(self, config_file="config.txt"):
        ...
        if (time.time() - os.path.getmtime(self.BOT_CONFIG["FOLLOWS_FILE"]) > 86400 or
            ...
            print("คำเตือน: ไฟล์ซิงค์ผู้ติดตาม Twitter ของคุณมีอายุมากกว่าหนึ่งวัน. "
                  "ขอแนะนำให้คุณซิงค์โดยการโทร sync_follows() "
                  "ก่อนดำเนินการต่อ.", file=sys.stderr)
    ...
    def auto_rt(self, phrase, count=100, result_type="recent"):
        ...
        for tweet in result["statuses"]:
            try:
                ...
                print("รีทวิส: %s" % (result["text"].encode("utf-8")), file=sys.stdout)
